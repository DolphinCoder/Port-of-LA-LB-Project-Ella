...
input_anacapa_path_1 <- here("data","fish","elas_fishcard_12S_all_taxonomy_tables","Summary_by_percent_confidence","60","fishcard_12S_all_ASV_raw_taxonomy_60.txt")
input_anacapa_path_2 <- here("data","fish","miu_fishcard_12S_all_taxonomy_tables","Summary_by_percent_confidence","60","fishcard_12S_all_ASV_raw_taxonomy_60.txt")

input_anacapa_path_3 <- here("data","fish","elas_c19_fishcard_taxonomy_tables","Summary_by_percent_confidence","60","c19_fishcard_ASV_raw_taxonomy_60.txt")
input_anacapa_path_4 <- here("data","fish","miu_c19_fishcard_taxonomy_tables","Summary_by_percent_confidence","60","c19_fishcard_ASV_raw_taxonomy_60.txt")
...
anacapa_table_1 %>% 
  dplyr::select(-`fishcard_12S_all_LA11.4.R3.S36.L001`,
         -`fishcard_12S_all_LA3.4.R3.S12.L001`,
         -`fishcard_12S_all_LB1.4.R3.S60.L001`) -> anacapa_table_1

#Generate Hash.key

colnames(anacapa_table_1) %>%
  str_remove("fishcard_12S_all_") %>% 
  str_remove("all_") -> new_colnames_1
  
colnames(anacapa_table_1) <- new_colnames_1

anacapa_table_1 %>% 
  mutate(., seq_number = str_replace(seq_number, "fishcard_12S_all_","fishcard_12S_all_elas_")) -> anacapa_table_1
...
colnames(anacapa_table_2) %>%
  str_remove("fishcard_12S_all_") %>% 
  str_remove("all_") -> new_colnames_2

colnames(anacapa_table_2) <- new_colnames_2

anacapa_table_2 %>% 
  mutate(., seq_number = str_replace(seq_number, "fishcard_12S_all_","fishcard_12S_all_miu_")) ->anacapa_table_2
...
anacapa_table_3 %>% 
  mutate(., seq_number = str_replace(fishcard_seq_number, "c19_fishcard_","c19_fishcard_elas_")) ->anacapa_table_3

anacapa_table_3 %>% 
  dplyr::select(seq_number, sum.taxonomy) -> hash.key_3

anacapa_table_4 %>% 
  mutate(., seq_number = str_replace(fishcard_seq_number, "c19_fishcard_","c19_fishcard_miu_")) ->anacapa_table_4

anacapa_table_4 %>% 
  dplyr::select(seq_number, sum.taxonomy) -> hash.key_4

rbind(hash.key_1,hash.key_2) -> hash.key
rbind(hash.key_3,hash.key_4) -> hash.key_c19

hash.key_c19 %>% 
  dplyr::select(c19_seq_number=seq_number,c19_sum.taxonomy=sum.taxonomy) -> hash.key_c19

cbind(hash.key,hash.key_c19) -> hashes_joined

hashes_joined %>% 
  mutate(., matcher = if_else(sum.taxonomy=="", c19_sum.taxonomy, sum.taxonomy)) -> hashes_joined

hashes_joined %>% 
  dplyr::select(seq_number,sum.taxonomy=matcher) -> hash.key
...
###Pre Occupancy Sum by Taxonomy, All PCR Tech Reps Separate Samples
  ...
  ASV.nested %>% 
    ...
    mutate(., barcode = case_when(str_detect(sample_name,"elas")~"Elas",
                                  str_detect(sample_name,"miu")~"Miu", 
                                  TRUE~"wrong")) %>% 
    separate(., sum.taxonomy, into= c("D","P","C","O","F","G","S"), sep=";", remove=F) -> holder
  ...
  ASV.nested %>% 
    ...
    filter(., barcode !="Elas") -> holder_new
